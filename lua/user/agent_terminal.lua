...
function Terminal.new(config)
  ...
  local defaults = {
    ...
    prefill_delay_ms = config.prefill_delay_ms or 700,
  }
  ...
  local function running(job_id) return job_id and job_id > 0 and vim.fn.jobwait({ job_id }, 0)[1] == -1 end

  local function current_root(bufnr)
    if vim.b[bufnr].zish_agent_root then return vim.b[bufnr].zish_agent_root end

    local path = vim.api.nvim_buf_get_name(bufnr)
    local start = path ~= "" and vim.fs.dirname(vim.fs.normalize(path)) or vim.fn.getcwd(0)
    -- Use the nearest repository or agent instruction directory as the project.
    return vim.fs.root(start, { ".git", "AGENTS.md", "CLAUDE.md" }) or start
  end
  ...
  local function configure_buffer(instance)
    ...
    vim.bo[bufnr].filetype = "zish_" .. config.name:lower() .. "_terminal"
    vim.b[bufnr].zish_agent_terminal = true
    vim.b[bufnr].zish_agent_root = instance.root
    vim.keymap.set("n", "ZZ", function() M.close(instance.root) end, { buffer = bufnr, desc = "Stop agent terminal" })
    vim.keymap.set(
      { "n", "t" },
      "<F7>",
      function() M.hide(instance.root) end,
      { buffer = bufnr, desc = "Hide agent terminal" }
    )
    ...
    for key in pairs(directions) do
      vim.keymap.set("t", "<M-" .. key .. ">", function() move_from_terminal(key) end, {
        buffer = bufnr,
        silent = true,
        desc = ("Leave %s terminal "):format(config.name) .. directions[key].smart,
      })
    end
    vim.keymap.set("n", "q", function() M.hide(instance.root) end, {
      ...
      silent = true,
      nowait = true,
      desc = ("Hide %s terminal"):format(config.name),
    })
    ...
    vim.api.nvim_create_autocmd("BufEnter", {
      ...
      desc = ("Enter %s terminal input mode"):format(config.name),
    })
  end
  ...
  local function new_instance(root, options)
    ...
    instance.job_id = vim.api.nvim_buf_call(instance.bufnr, function()
      return vim.fn.jobstart(options.command, {
        cwd = root,
        term = true,
        on_exit = function() instance.job_id = nil end,
      })
    end)
    if not instance.job_id or instance.job_id <= 0 then
      M.close(root)
      vim.notify(("Could not start %s terminal"):format(config.name), vim.log.levels.ERROR)
      return nil
    end
  ...
  local function merged_options(options)
    ...
    if not options.command then options.command = config.command(options.root) end
    return options
  end
  ...
  local function inject_context(instance, context, options)
    ...
    local function send()
      if instances[instance.root] ~= instance or instance.attached_context ~= key or not running(instance.job_id) then
        return
      end
      local prompt = config.bracketed_paste and "\27[200~" .. context.prompt .. "\27[201~" or context.prompt
      vim.fn.chansend(instance.job_id, prompt)
    end
  ...
  function M.open(options)
    ...
    if not save_source_buffer(source_bufnr) then
      vim.notify(
        ("Could not save the current file; %s was not opened with stale content"):format(config.name),
  ...
  function M.attach_current(options)
    local source_bufnr = vim.api.nvim_get_current_buf()
    if not save_source_buffer(source_bufnr) then
      vim.notify(
        ("Could not save the current file; %s context was not updated"):format(config.name),
        vim.log.levels.ERROR
      )
      return nil
    end
    ...
    options = merged_options(vim.tbl_extend("force", options or {}, { root = context.root }))
  ...
  function M.exit_current_editor()
    ...
    local root = current_root(bufnr)
    if vim.b[bufnr].zish_agent_terminal then return M.close(root) end
  ...
  function M.toggle()
    ...
    if instance and valid_window(instance.winid) then
      if vim.api.nvim_get_current_win() == instance.winid then return M.hide(root) end
      vim.api.nvim_set_current_win(instance.winid)
      pcall(vim.cmd.startinsert)
      return instance
    end
  ...
  function M.setup()
    ...
    configured = true
    vim.api.nvim_create_user_command(("%sTerminal"):format(config.name), function() M.toggle() end, {
      desc = ("Toggle raw %s terminal split"):format(config.name),
    })
    vim.api.nvim_create_user_command(("%sTerminalClose"):format(config.name), function() M.close(current_root(0)) end, {
      desc = ("Stop the current project's %s terminal"):format(config.name),
    })
    vim.api.nvim_create_user_command(("%sAttach"):format(config.name), function() M.attach_current() end, {
      desc = ("Open %s and attach the current file"):format(config.name),
    })
    if config.map_exit then
      vim.keymap.set("n", "ZZ", M.exit_current_editor, {
        silent = true,
        desc = ("Save and exit with the project %s terminal"):format(config.name),
      })
    end

    vim.api.nvim_create_autocmd("VimLeavePre", {
      group = vim.api.nvim_create_augroup(("User%sTerminalLifecycle"):format(config.name), { clear = true }),
      callback = function()
      ...
      desc = ("Stop embedded %s jobs before Neovim exits"):format(config.name),
    })
  end
